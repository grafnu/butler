...
To prevent infinite feedback loops and minimize bus traffic, components MUST ignore incoming messages where the `source` field in the envelope matches their own `source` identifier. While the Deduplication rule (Section 7.3) allows processing of self-messages for specific local state synchronization, components MUST NOT engage in behavior where processing a self-originated message triggers the publication of a new message that could sustain a loop.

**Critical Note:** This is especially important for Orchestrators that subscribe to registry-wide or system-wide topics (e.g., `/uufi/r/+/c/config/cloud`), as they will receive their own updates. Strict adherence to this rule is essential for system stability.
...
- **Redundancy Rule:** Implementations MUST reject messages where envelope fields duplicate topic-encoded data (e.g., `deviceId` in a device-specific topic, or `subType`/`subFolder` in any UUFI topic). "Reject" means the message MUST be logged as a protocol violation and its payload MUST NOT be processed. This rule prevents ambiguity and ensures that the topic remains the authoritative source of routing information.
