...
#### 10.1.3.1. Unified Broker Setup under Secure Connection Schemes
The setup utility MUST NOT bypass broker connectivity checks or broker startup when a secure connection prefix (`mqtts://` or `ssl://`) is provided. Secure connection prefixes represent encrypted MQTT broker endpoints, which still require starting a local broker if one is not already active on the configured port. To ensure a unified and compliant setup:
1. **Scheme/Protocol Normalization:** When evaluating whether to run connectivity checks or launch `start_local`, the setup utility MUST extract and normalize the protocol scheme. Both secure schemes (`mqtts://`, `ssl://`) and insecure schemes (`mqtt://`) MUST be treated as MQTT-based transports that require an active Mosquitto broker on the specified port.
2. **Explicit Broker Startup:** If the normalized protocol requires a local broker, and no broker is currently listening on the target port, the setup utility MUST execute `start_local` with the appropriate arguments.
3. **No Scheme-Based Bypass:** Programmatic bypasses that restrict setup checks or broker startup strictly to the unencrypted `"mqtt"` scheme (e.g., `if conn["scheme"] == "mqtt":` or `if conn_spec.protocol == "mqtt":`) are considered protocol violations. Setup utilities MUST invoke the `start_local` utility for any requested MQTT transport, secure or insecure, unless the connection specification protocol is explicitly recognized as non-broker-based (e.g., `pubsub://`).

#### 10.1.3.2. Programmatic Configuration Adaptation for Dynamic Ports and Workspace-Relative Certificates
To support running inside hermetic, sandboxed, or isolated dynamic-testing environments (such as the cross-implementation test matrix) without requiring root privileges or accessing global system directories, the setup utility MUST programmatically adapt and update local configurations before spawning any background processes:
1. **Dynamic Port Adaptation:** The setup utility MUST locate all instance-specific configuration files (specifically `impl/udmi/udmis/etc/local_pod.json`, `impl/udmi/etc/local_pod.json`, and `impl/udmi/etc/mosquitto_udmi.conf`) and programmatically update any default port definitions (such as `8883` or `1883`) to match the target dynamic port passed via the `<conn_spec>` or `MQTT_PORT` environment variables. 
2. **Workspace-Relative Certificate Resolution:** In isolated, non-privileged environments, global system paths (such as `/etc/mosquitto/certs/ca.crt`) are inaccessible or invalid. The setup utility MUST programmatically rewrite any global certificate paths inside `local_pod.json` to point to the workspace-relative isolated certificate directory (`impl/udmi/var/mosquitto/certs/`). This guarantees that UDMIS services (e.g., started by `start_local`) can successfully load and authenticate against the active Mosquitto broker on the custom port without experiencing connection timeouts or file permission failures.
3. **Template Preservation:** To maintain idempotency across multiple setup cycles, the setup utility SHOULD preserve original configuration files as templates or perform clean, safe, sequential replacements that do not corrupt JSON syntax.

#### 10.1.3.3. Mandatory Event-Loop and Callback Type Verification
To prevent untested code paths from introducing runtime crashes when switching from individual test harnesses (`smokeit`) to full orchestration modes (`butler` / `run_cross`):
1. **Complete API Verification:** All internal callbacks (such as `on_connect` or packet receipt handlers) MUST be fully verified for syntactic and semantic correctness. Under no circumstances should runtime files contain unused, legacy, or incorrect function calls (such as calling a 4-argument `build_topic` function with only 3 arguments, e.g., `sub_topic = build_topic("+", "+", "#")`).
2. **Callback Safety:** Any parameters or helper invocations within system callbacks must be structurally covered. If a helper function is unused or legacy, it MUST be removed entirely to prevent runtime `TypeError` or `NameError` failures when client handshakes are triggered.
...
All assets MUST be stored within the designated git-ignored temporary directories (such as `tmp/var/mosquitto/certs/` or `impl/udmi/var/mosquitto/certs/`). Expecting or requiring other extensions (such as `.key` instead of `.pem`) is strictly prohibited. To prevent loading stale or cached certificates from leftover `tmp/` directories from prior historical or standalone runs, clients MUST prioritize searching and loading active certificates from the newly generated local UDMI directory (`impl/udmi/var/mosquitto/certs/`) BEFORE falling back to any workspace-level or temporary directory paths (such as `tmp/var/mosquitto/certs/`). Reversing this precedence order or failing to verify the freshness of the `tmp/` certificates is a protocol violation. Additionally, the setup utility MUST cleanly purge and remove any pre-existing or stale certificates inside `tmp/var/mosquitto/certs/` upon initialization to prevent directory interference and stale credential usage.
When establishing any secure client connection (such as over `mqtts://` or `ssl://` schemes) to a local broker that requires client authentication/verification, both the orchestrator and verifier clients MUST load and supply the Client Public Certificate (`rsa_private.crt`) and Client Private Key (`rsa_private.pem`) in addition to the Certificate Authority (CA) Certificate (`ca.crt`) using the client library's standard TLS APIs (such as `tls_set()` with all three arguments in Python's `paho-mqtt` library). Relying solely on the Certificate Authority (CA) Certificate without presenting client-side certificates is strictly prohibited.

- **Connection Scheme Recognition**: All implementations' connection specification parsing utilities MUST explicitly recognize and support secure schemes (`mqtts://` and `ssl://`) in addition to insecure ones (`mqtt://` and `pubsub://`) as valid connection prefixes. They MUST NOT fall back to unencrypted defaults or treat these secure prefixes as functional positional arguments.
- **Connection Specification Parsing & Credential Extraction**: When parsing a connection specification string (e.g., `mqtts://username:password@host:port/`), implementations MUST correctly extract and separate the `username` and `password` components (if present) from the host/port part. They MUST NOT treat the entire userinfo string (such as `username:password`) as a single username. They MUST NOT include the password or password delimiter (`:`) in any generated client IDs, reported principals, or derived project spec strings. If username/credentials are not specified, or if they match the active branch name, implementation ID, or "unknown", they MUST default to `rocket` and `monkey` as the standard credentials for MQTT basic authentication and dynamic security, ensuring compatibility with the active broker's configured roles.
- **Cross-Implementation Certificate Sharing**: In a multi-implementation cross-testing environment (where a verifier from implementation `v` orchestrates a test matrix against a butler from implementation `b`), the target butler client running in `impl/b` MUST be provided access to the broker's active Certificate Authority (CA) and client certificates. The cross-testing harness (`tools/run_cross`) MUST dynamically symlink or map the target butler's local certificate temporary directories (`impl/b/impl/udmi/var/mosquitto/certs/`) to the active verifier/broker's certificate folder (`impl/v/impl/udmi/var/mosquitto/certs/`) to ensure the target butler client can dynamically load and present trusted, valid certificates signed by the active broker's CA.
...
*   **Non-Privileged/Non-Sudo Execution:**
    To ensure safe, permission-isolated execution inside sandboxes without requiring administrative or root privileges, all tools and background scripts MUST automatically operate entirely in non-privileged (no-sudo) user-space whenever a specific, non-default port is configured or passed. Bypassing privilege restrictions using `sudo` is strictly prohibited. Under no circumstances should any `sudo` simulation wrapper or privilege elevation bypass be used. If there is any case where `sudo` or administrative privilege elevation is still called, required, or executed, the system MUST immediately flag it as a fundamental compatibility error and fail-fast.

*   **Repository and Workspace Cleanliness & Prohibition of Temporary/Mock Executables:**
    To maintain complete system integrity and clean workspaces, **temporary executables, mock executables, or otherwise not-real/simulated binaries are strictly prohibited in all cases.** The system MUST either function exactly as documented using the standard, real platform-native executables, or fail hard immediately. Under no circumstances should any tool, script, or setup utility create or utilize temporary binaries, wrappers, or simulation scripts (such as mock `ps` or mock `java` wrappers) to bypass system environment checks or standard OS commands.
    If a problem cannot be solved using the real resources and structure outlined in these specifications, it MUST result in a hard, fatal failure.
    For permissible non-executable runtime files (such as dynamically created directories, runtime logs, configuration overlays, and virtual environments, e.g., `tmp/var/`, `tmp/etc/`, `tmp/venv/`), they MUST be created and stored exclusively inside pre-defined git-ignored locations (specifically within the `tmp/` or `testing/` directories) to ensure no run-time artifacts are tracked by git. Storing any runtime files outside of these pre-defined `.gitignore` paths is strictly prohibited.
...
Simulated devices and DUTs MUST report their actual software and firmware states using the subsystem ID `"system"` (rather than `"main"` or other custom names) to match the expected version configured under `"system"`. Rather than defining redundant structural paths locally, all message structures, nesting, and payload formats (including those under `blobset`) MUST strictly follow the authoritative UDMI schemas (defined under `impl/udmi/schema/`) and the UUFI bus transport specification (`impl/udmi/docs/specs/uufi.md`). Sourcing, parsing, or extracting of actual software versions MUST be done strictly from the standard `"system.software.<subsystem>"` path of the device state payload (specifically `"system.software.system"` for the system subsystem). Sourcing or parsing actual versions from any other path (such as `"blobset.blobs.<subsystem>.version"`) is strictly prohibited. Local Butler components are concerned strictly with identifying the `"system"` subsystem name and extracting the corresponding values from these standard schema-defined locations.
