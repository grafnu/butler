...
Simulated devices and DUTs MUST report their actual software and firmware states using the subsystem ID `"system"` (rather than `"main"` or other custom names) to match the expected version configured under `"system"`. Rather than defining redundant structural paths locally, all message structures, nesting, and payload formats (including those under `blobset`) MUST strictly follow the authoritative UDMI schemas (defined under `impl/udmi/schema/`) and the UUFI bus transport specification (`impl/udmi/docs/specs/uufi.md`). Local Butler components are concerned strictly with identifying the `"system"` subsystem name and extracting the corresponding values from these standard schema-defined locations.
<!-- ASSUMPTION: Relegating structural specifications entirely to the official UDMI schemas and UUFI transport standards keeps the local Butler specification clean and structurally decoupled. -->
