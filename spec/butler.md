...
*   **Repository and Workspace Cleanliness & Prohibition of Temporary/Mock Executables:**
    To maintain complete system integrity and clean workspaces, **temporary executables, mock executables, or otherwise not-real/simulated binaries are strictly prohibited in all cases.** The system MUST either function exactly as documented using the standard, real platform-native executables, or fail hard immediately. Under no circumstances should any tool, script, setup utility, or agent create or utilize temporary binaries, shell script wrappers (such as replacing platform binaries like `bin/mosquitto` with shell scripts), or simulation scripts (such as mock `ps` or mock `java` wrappers) to bypass system environment checks or standard OS commands.
    Furthermore, agents MUST NOT attempt to diagnose, debug, or manipulate environment variables like `LD_LIBRARY_PATH`, resolve missing shared library dependencies (such as `libdlt.so.2`), or patch binary line endings on platform utilities. Any low-level system, binary, or environment dependency failure MUST result in an immediate, hard fail-fast error without attempting local patching or diagnostic loops.
