...
- **Handshake Compliance (Client-Server Separation):** Butler acts strictly as a gateway client. On startup, it MUST publish its own handshake request (`{implementation_id}.butler` or `{implementation_id}.butler.setup`) to `/uufi/c/state/udmi` with a secure transaction ID, and block until it receives a valid config handshake reply from UDMIS on `/uufi/c/config/udmi`. Only after this handshake completes should the Butler subscribe to the UUFI topic tree and begin processing messages. Sourcing and responding to handshake requests from other clients (Devices and Verifiers) is completely outsourced to the UDMIS gateway, resolving duplicate-responder race conditions.
- **State Machine:**
...
- **Gateway Handshake and Blob Update Separation:** The device launches and performs its gateway handshake with UDMIS first, completely decoupled from the software update sequence. Sourcing and responding to handshake requests is entirely managed by UDMIS as the sole handshake server.
- **Blob Update Sequence:** The actual coordination of the firmware update (using `/config/blobset` payloads) is managed solely as a separate sequence between the Butler and the Device Under Test (DUT), negotiated directly using `/config/blobset` and `/state/system` (completely decoupled from the infrastructure-level gateway handshake).
...
### 5.1 Update Flow
1. **Initiation & Handshake Gating:** The test harness (`tools/run_cross` or `smokeit`) MUST poll for and confirm the successful handshake completion of **both** the verifier (`Handshake completed for impl_A.verifier`) and the butler (`Handshake completed for impl_A.butler`) before executing the `site_trigger` utility. This ensures both monitoring and orchestration engines are fully connected and subscribed before the model update is published. Once gated, the expected/desired version is updated in the live Cloud Model via the UDMI `site_trigger` utility (Scope 3) which updates the local model file on disk and publishes a corresponding `model/cloud` Model Update message over the UUFI bus, emulating a database update.
...
To ensure strict protocol safety and prevent specification divergence, all Butler and Verifier implementations MUST enforce strict compliance with the authoritative UDMI JSON schemas (specifically `state_system.json`). Software subsystem states are parsed as flat string version primitives (e.g., `"system": "v1"`), or as valid JSON objects containing the `"version"` and optionally `"status"` attributes.
...
| **REQ-HSK-003** | Handshake | Butler Handshake Client Compliance: Butler strictly acts as a gateway client. On startup, it MUST publish its own handshake request (`{implementation_id}.butler` or `{implementation_id}.butler.setup`) to `/uufi/c/state/udmi` with a secure transaction ID, and block until it receives a valid config handshake reply from UDMIS on `/uufi/c/config/udmi`. Only after this handshake completes should the Butler subscribe to the UUFI topic tree and begin processing messages. | Unit / Integration Test |
