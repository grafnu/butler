...
3. **Hermetic Teardown (`--stop` flag):** Teardown background processes using PID-file tracking (`out/mosquitto.pid`, `out/etcd.pid`) without sweeping process termination commands.
4. **Fail-Fast Policy for Low-Level System Errors:** As specified in Section 10.1.3.1 below, low-level platform, shared library (`LD_LIBRARY_PATH`), or shell wrapper creation attempts MUST result in an immediate hard fail.

#### 10.1.3.1. Repository Cleanliness & Fail-Fast System Policy
To maintain workspace integrity:
- **Prohibition of Temporary/Mock Executables & Wrappers:** Temporary executables, mock binaries, or shell script wrappers (such as replacing platform binaries like `bin/mosquitto` with shell scripts) are strictly prohibited. The system MUST run native platform binaries directly or fail immediately.
- **Prohibition of Environment Debugging:** Agents MUST NOT attempt to diagnose or manipulate `LD_LIBRARY_PATH`, resolve missing shared libraries (e.g., `libdlt.so.2`), or patch binary line endings on system utilities. Any low-level binary or system failure MUST result in an immediate hard fail-fast error.
