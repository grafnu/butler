...
1. **Environment Setup:**
   ...
   - **Explicit Broker Invocation:** The local UDMI broker startup utility (`impl/udmi/bin/start_local`) MUST be invoked directly by passing the local site model directory path (e.g., `testing/udmi_site_model`) as its first command-line argument, and the connectivity specification URL (e.g., `mqtt://localhost:$mqtt_port/`) as its second command-line argument:
     ```bash
     impl/udmi/bin/start_local testing/udmi_site_model mqtt://localhost:$mqtt_port/
     ```
     Tools and update scripts MUST NOT inspect, read, or parse the internal contents of `start_local` or other local setup utilities to determine port configurations; they must rely strictly on passing these arguments directly on the command line.
   - **Non-Privileged/Non-Sudo Execution (`UDMI_NO_SUDO=true`):** To run in isolated, sandboxed environments without root privileges, the environment variable `UDMI_NO_SUDO=true` MUST be exported or prepended to all setup, test, and run commands. This instructs the cloned UDMI utilities to start mosquitto and etcd directly in user-space on the negotiated ports, completely bypassing any `sudo` or `systemctl` commands.
