...
1. **Environment Setup:**
   ...
   - **Non-Privileged/Non-Sudo Execution:** To run in isolated, sandboxed environments without root privileges, all tools and background scripts MUST automatically operate entirely in non-privileged (no-sudo) user-space whenever a specific, non-default port is configured or passed, completely bypassing any `sudo` or `systemctl` commands. Under no circumstances should any `sudo` simulation wrapper or privilege elevation bypass be used. If any `sudo` or privilege elevation is still called, required, or executed, the system MUST immediately flag it as a fundamental compatibility error and fail-fast.
