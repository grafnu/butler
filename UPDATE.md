...
1. **Environment Setup:**
   ...
   - **Explicit Broker Invocation:** The local UDMI broker startup utility (`impl/udmi/bin/start_local`) MUST be invoked directly by passing the connectivity specification URL (e.g., `mqtt://localhost:$mqtt_port/`) as its first command line argument. Tools and update scripts MUST NOT inspect, read, or parse the internal contents of `start_local` or other local setup utilities to determine port configurations; they must rely strictly on passing the connection specification argument directly on the command line.
