...
class MockDevice:
    def __init__(self, conn_spec, registry_id, device_id, fail_mode=False):
        ...
        dev_info = self.model_repo.get_device_state(self.registry_id, self.device_id)
        if dev_info:
            ...
            for reg_id, reg_data in registries.items():
                ...
                for dev_id, info in devices.items():
                    ...
                    if target is not None:
                        self.model_repo.set_target_version(reg_id, dev_id, target)
                    if current is not None or lkg is not None or status is not None:
                        self.model_repo.update_current_version(reg_id, dev_id, current, lkg_version=lkg, status=status)
            self.push_model()
