...
class ButlerOrchestrator:
    ...
    def on_message(self, topic, device_id, sub_type, sub_folder, data):
        if not self.bus.handshake_complete:
            # Handshake is handled in ButlerBusBase._handle_received_message
            return
    ...
    def handle_cloud_reply(self, data):
        ...
        cloud_data = payload_wrapper.get("cloud", {})
    ...
    def handle_device_state(self, device_id, data):
        ...
        payload_wrapper = data.get("payload", {})
        update_state = payload_wrapper.get("update", {})
        ...
        # Always trust and persist LKG if reported (3.3)
        ...
        old_status = device_info.get("status")
        ...
        if status == "pending":
            ...
            if current_version != target_version:
                ...
                if device_info.get("status") != "quiescent" or device_info.get("current_version") != current_version:
                    print(f"[butler] Device {sub_key} is quiescent and compliant.")
                    updates["current_version"] = current_version
                    updates["status"] = "quiescent"

        elif status == "success":
            if device_info.get("current_version") != current_version or device_info.get("status") != "quiescent":
    ...
    def update_model(self, registry_id, device_id, subsystem="main", **detail):
        ...
        # Partial Merge Update (3.2): MUST NOT send the target_version unless they explicitly intend to change it.
        send_detail = detail.copy()
        if "target_version" not in detail:
            if "target_version" in send_detail: del send_detail["target_version"]
    ...
    def trigger_update(self, registry_id, device_id, device_info):
        ...
        self.update_model(registry_id, device_id, subsystem=subsystem, status="active", active_version=version)
    ...
    def trigger_rollback(self, registry_id, device_id, device_info):
        ...
        # Update local cache and model
        self.last_action_time[sub_key] = time.time()
        self.update_model(registry_id, device_id, subsystem=subsystem, target_version=lkg, status="error")
    ...
    def reconcile_device(self, registry_id, device_id, subsystem_id="main"):
        ...
        # Settling time check (3.3): prevent re-evaluation within 5s
        now = time.time()
        last_action = self.last_action_time.get(sub_key, 0)
        if now - last_action < self.settling_time:
            return

        current = device_info.get("current_version") or ""
        target = device_info.get("target_version")
        status = device_info.get("status")

        if current != target:
            print(f"[butler] Reconciling {sub_key}: current={current}, target={target}, status={status}")
            # Re-triggering Logic (3.3):
            if status in ["quiescent", "error", None]:
                self.trigger_update(registry_id, device_id, device_info)
            elif status == "active" or sub_key in self.devices_pending:
    ...
    def check_timeouts(self):
        while True:
            ...
            for sub_key, start_time in list(self.devices_pending.items()):
                if now - start_time > self.timeout: # Default 60s (3.3)
                    parts = sub_key.split("/")
                    if len(parts) == 3:
...
def main():
    ...
    if not orchestrator.bus.wait_for_handshake(timeout=60): # 60s handshake timeout (3.3)
