...
class Orchestrator:
    def __init__(self, conn_spec, fail_mode=False):
        ...
        self.pending_updates = {} # (registry_id, device_id): {timestamp, target_version}
        self.settle_times = {} # (registry_id, device_id): timestamp
    ...
    def on_message(self, env, payload, topic, raw=None):
        ...
        if sub_folder == "cloud" and sub_type == "config":
            ...
            for reg_id, reg_data in registries.items():
                ...
                for dev_id, dev_data in devices.items():
                    ...
                    self.models[reg_id][dev_id].update(dev_data)
        ...
        if sub_type == "state" and sub_folder == "update":
            ...
            self.update_cloud_model(registry_id, device_id, current_version=current_version, lkg_version=lkg_version, status=status)
            ...
            if status in ["success", "failure"]:
                ...
                if status == "failure":
                    ...
                    self.rollback_cloud_model(registry_id, device_id)
    ...
    def update_cloud_model(self, registry_id, device_id, target_version=None, current_version=None, lkg_version=None, status=None):
        dev_data = {}
        if target_version: dev_data["target_version"] = target_version
        if current_version is not None: dev_data["current_version"] = current_version
        if lkg_version: dev_data["lkg_version"] = lkg_version
        if status is not None: dev_data["status"] = status
    ...
    def rollback_cloud_model(self, registry_id, device_id):
        ...
        self.update_cloud_model(registry_id, device_id, target_version=lkg, status=status)
        ...
        for registry_id, devices in self.models.items():
            for device_id, info in devices.items():
                ...
                if retrigger:
                    ...
                    subsystem = "main" # Default subsystem for fetch
