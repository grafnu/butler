...
class ButlerVerifier:
    ...
    def validate_message(self, data, is_strict=False):
        # Validation Schema: mandatory UDMI payload fields (within 'payload' wrapper)
        errors = []
        payload_wrapper = data.get("payload", {})

        timestamp = payload_wrapper.get("timestamp")
        if not timestamp:
            errors.append("Missing mandatory 'timestamp' in payload wrapper")
        elif is_strict:
        ...
        if "version" not in payload_wrapper:
            errors.append("Missing mandatory 'version' in payload wrapper")
    ...
    def on_message(self, topic, device_id, sub_type, sub_folder, data):
        ...
        if sub_type == "state" and sub_folder == "update":
            ...
            cloud_block = payload_wrapper.get("cloud", {})
            operation = cloud_block.get("operation")
