...
class ButlerBusBase:
    ...
    def _handle_received_message(self, topic, device_id, sub_type, sub_folder, udmi_payload):
        ...
        # Handshake check (look inside 'udmi' subfolder within 'payload' wrapper)
        if sub_type == "config" and sub_folder == "udmi":
            payload_wrapper = udmi_payload.get("payload", {})
            udmi_block = payload_wrapper.get("udmi", {})
    ...
    def _wrap_payload(self, payload_data, sub_folder):
        # Minimal precision RFC 3339 (9.4)
        ...
        # Use a distinct key for UDMI version to avoid conflict with firmware version
        udmi_version = payload_data.get("udmi_version", "1.5.2")

        inner_data = payload_data.copy()
        if "udmi_version" in inner_data: del inner_data["udmi_version"]

        if not sub_folder:
             return inner_data, publish_time

        if sub_folder in inner_data:
            actual_inner = inner_data[sub_folder]
        else:
            actual_inner = inner_data

        wrapped = {
            "timestamp": timestamp,
            "version": udmi_version,
            sub_folder: actual_inner
        }
    ...
    def start_handshake(self, device_id=None):
        ...
        self.subscribe_uufi()
        self.publish_uufi(None, "state", payload["udmi"], "udmi", transaction_id=self.handshake_transaction_id)
...
class ButlerMQTTBase(ButlerBusBase):
    ...
    def _on_mqtt_message(self, client, userdata, msg):
        try:
            ...
            # Raw wire format support for human/automated monitoring (9.3)
            ...
            try:
                ...
                # Redundancy Rule (9.3): Reject messages that contain redundant envelope fields already present in the topic
                if registry_id and "deviceRegistryId" in data:
                    print(f"[{self.source}] REJECTED: Redundant deviceRegistryId in envelope")
                    return
                if device_id and "deviceId" in data:
                    print(f"[{self.source}] REJECTED: Redundant deviceId in envelope")
                    return
                if "subType" in data:
                    print(f"[{self.source}] REJECTED: Redundant subType in envelope")
                    return
                if "subFolder" in data:
                    print(f"[{self.source}] REJECTED: Redundant subFolder in envelope")
                    return

                # Mandatory payload wrapper check (9.1)
                if "payload" not in data:
                    print(f"[{self.source}] REJECTED: Missing nested 'payload' key")
                    return

                # Build a unified message for internal use
                udmi_payload = data.copy()
                if registry_id:
                    udmi_payload["deviceRegistryId"] = registry_id
                if device_id:
                    udmi_payload["deviceId"] = device_id

                udmi_payload["subType"] = sub_type
                udmi_payload["subFolder"] = sub_folder

                self._handle_received_message(msg.topic, device_id, sub_type, sub_folder, udmi_payload)
            except (json.JSONDecodeError, IndexError):
                pass
        except Exception as e:
            print(f"[{self.source}] Error processing MQTT message: {e}")
            pass
    ...
    def publish_uufi(self, device_id, sub_type, payload_data, sub_folder=None, direction="reflect", target_principal=None, transaction_id=None, registry_id=None):
        ...
        # Redundancy Rule (9.3/4.1): Envelope MUST NOT include data already in the topic
        envelope = {
...
class ButlerPubSubBase(ButlerBusBase):
    ...
    def _callback(self, message):
        try:
            payload_wrapper = json.loads(message.data.decode("utf-8"))
            attributes = dict(message.attributes)

            # For PubSub, the data IS the payload wrapper.
            udmi_payload = attributes.copy()
            udmi_payload["payload"] = payload_wrapper
