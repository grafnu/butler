...
def main():
    ...
    reg = subprocess.run(["bin/register", registry_id, device_id], capture_output=True, text=True)
    if reg.returncode != 0:
        print(f"Register failed:\n{reg.stderr}")
        cleanup([observe_proc, verifier_proc, butler_proc])
        sys.exit(1)

    mocket_proc = subprocess.Popen(["bin/mocket", conn_spec, registry_id, device_id], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)
    time.sleep(3)
    ...
    trig = subprocess.run(["bin/trigger", registry_id, device_id, "1.1.0", fw_path], capture_output=True, text=True)
