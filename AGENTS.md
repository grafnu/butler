...
3. Sandbox isolation is strictly for the purposes of the butler implementation itself. It does not apply to the analysis or execution of other platform-level tools (such as `start_local` or other external scripts). As long as the specific butler implementation is compliant and correct, this sandbox isolation stipulation is fully met. The fact that an external tool might violate these isolation guidelines or exhibit external environmental defects does not prevent the agent from operating.
